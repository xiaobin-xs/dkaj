...
from modelsR import CauseSpecificCox
...
for experiment_idx in range(n_experiment_repeats):
    for dataset in datasets:
        ...
        X_train_np, preprocessor = build_preprocessor_and_preprocess(X_train_raw_np, cox='csCox')
        X_val_np = apply_preprocessor(X_val_raw_np, preprocessor, cox='csCox')
        ...
        model = CauseSpecificCox(n_causes=len(events))
        ...
        X_test_np = apply_preprocessor(X_test_raw_np, preprocessor, cox='csCox')
        ...
        cif_pred_all_events = np.clip(cif_pred_all_events, 0., 1.) # to deal with weird numerical issues origin from riskRegression.predictRisk
