...
from modelsR import CompetingRiskRSF
...
hyperparams = \
    [(ntree, mtry, nodesize, nodedepth, ntime)
     for ntree
     in ast.literal_eval(config[method_header]['ntree'])
     for mtry
     in ast.literal_eval(config[method_header]['mtry'])
     for nodesize
     in ast.literal_eval(config[method_header]['nodesize'])
     for nodedepth
     in ast.literal_eval(config[method_header]['nodedepth'])
     for ntime
     in ast.literal_eval(config[method_header]['ntime'])]
...
for experiment_idx in range(n_experiment_repeats):
    for dataset in datasets:
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam_idx, hyperparam in enumerate(tqdm(hyperparams, desc="Hyperparams")):
                tqdm.write(f"Running config {hyperparam_idx}: {hyperparam}")
                ntree, mtry, nodesize, nodedepth, ntime = hyperparam
                ...
                model = CompetingRiskRSF(ntree=ntree, mtry=mtry, nodesize=nodesize,
                                         nodedepth=nodedepth, ntime=ntime, seed=hyperparam_random_seed,
                                         n_causes=len(events))
                time_elapsed_filename = \
                    os.path.join(output_dir, 'models',
                                 '%s_%s_exp%d_nt%f_mt%s_ns%d_nd%s_nt%d_'
                                 % (estimator_name, dataset, experiment_idx,
                                    ntree, mtry, nodesize, nodedepth, ntime)
                                 + 'time.txt')
                ...
                new_hyperparam = (ntree, mtry, nodesize, nodedepth, ntime, 
            ...
            ntree, mtry, nodesize, nodedepth, ntime, seed = arg_min
            model_filename = \
                os.path.join(output_dir, 'models',
                             '%s_%s_exp%d_nt%f_mt%s_ns%d_nd%s_nt%d_'
                                % (estimator_name, dataset, experiment_idx,
            ...
            print('Loading previous chosen best hyperparam and fitting on the training data...', flush=True)
        ...
        model = CompetingRiskRSF(ntree=ntree, mtry=mtry, nodesize=nodesize,
                                 nodedepth=nodedepth, ntime=ntime, seed=seed, n_causes=len(events))
