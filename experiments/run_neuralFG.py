...
from models import NeuralFineGrayWrap
...
activation = str(config[method_header]['activation'])

hyperparams = \
    [(dropout, n_layers, n_layers_surv, n_nodes, batch_size, lr)
     for dropout
     in ast.literal_eval(config[method_header]['dropout'])
     for n_layers
     in ast.literal_eval(config[method_header]['n_layers'])
     for n_layers_surv
     in ast.literal_eval(config[method_header]['n_layers_surv'])
     for n_nodes
    ...
    for dataset in datasets:
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam_idx, hyperparam in enumerate(hyperparams):
                dropout, n_layers, n_layers_surv, n_nodes, batch_size, lr = hyperparam
                ...
                model = NeuralFineGrayWrap(layers=[n_nodes for _ in range(n_layers)], 
                                           layers_surv=[n_nodes for _ in range(n_layers_surv)],
                                           dropout=dropout, act=activation)
                time_elapsed_filename = \
                    os.path.join(output_dir, 'models',
                                 '%s_%s_exp%d_do%f_nla%d_nlas%d_nno%d_'
                                 % (estimator_name, dataset, experiment_idx,
                                    dropout, n_layers, n_layers_surv, n_nodes)
                                 +
                ...
                new_hyperparam = (dropout, n_layers, n_layers_surv, n_nodes, 
                                  batch_size, lr, hyperparam_random_seed)
            ...
            dropout, n_layers, n_layers_surv, n_nodes, batch_size, lr, seed = arg_min
            model_filename = \
                os.path.join(output_dir, 'models',
                             '%s_%s_exp%d_do%f_nla%d_nlas%d_nno%d_'
                             % (estimator_name, dataset, experiment_idx,
