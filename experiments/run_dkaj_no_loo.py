...
LOO = False # whether to use leave-one-out when computing the kernel weights in DKAJ
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
estimator_name = 'survboost'
finetune_estimator_name = 'dkaj10_noLoo'
...
warmstart_hyperparam_hash.update(str(n_random_times_per_data_point).encode('utf-8'))
...
finetune_hyperparam_hash.update(str(n_random_times_per_data_point).encode('utf-8'))
...
for experiment_idx in range(n_experiment_repeats):
    for dataset in datasets:
        ...
        survival_boost_leaves_train_np = \
            survival_boost_leaves_train_np.reshape(n_random_times_per_data_point, X_train_np.shape[0], -1)\
                .transpose(1,0,2).reshape(X_train_np.shape[0], -1)
        survival_boost_leaves_val_np = \
            hist_gradient_boosting_classifier_apply(survival_boost.estimator_,
                                                    X_val_with_random_times_np,
                                                    os.cpu_count())#.reshape(X_val_np.shape[0], -1)
        survival_boost_leaves_val_np = \
            survival_boost_leaves_val_np.reshape(n_random_times_per_data_point, X_val_np.shape[0], -1)\
                .transpose(1,0,2).reshape(X_val_np.shape[0], -1)
        ...
        if not os.path.isfile(warmstart_best_params_filename) \
            ...
            for warmstart_hyperparam in warmstart_hyperparams:
                ...
                warmstart_model_filename = \
                    model_filename[:-6] \
                    + '_tuna_mlp_sr%f_nla%d_nno%d_bs%d_mnep%d_lr%f_nt%d.pt' \
                    % (squared_radius, n_layers, n_nodes, batch_size,
                       max_n_epochs, lr, n_random_times_per_data_point)
        ...
        warmstart_model_filename = \
            model_filename[:-6] \
            + '_tuna_mlp_sr%f_nla%d_nno%d_bs%d_mnep%d_lr%f_nt%d.pt' \
            % (squared_radius, n_layers, n_nodes, batch_size, max_n_epochs, lr, n_random_times_per_data_point)
        warmstart_checkpoint = torch.load(warmstart_model_filename)
        base_neural_net.load_state_dict(warmstart_checkpoint['model_state_dict'])
        optimizer = torch.optim.Adam(base_neural_net.parameters(), lr=lr)
        ...
        if not os.path.isfile(output_train_metrics_filename) or \
            ...
            for hyperparam_idx, hyperparam in enumerate(finetune_hyperparams):
                ...
                dkaj_model = DKAJ(base_neural_net, device=device, alpha=alpha, sigma=sigma, beta=beta, tau=np.sqrt(-np.log(min_kernel_weight)), loo=LOO)
                ...
                finetune_model_filename = \
                    os.path.join(
                        ...
                        +
                        'nn%d_nla%d_nno%d_bs%d_mnep%d_lr%f_nt%d.pt'
        ...
        model_filename = \
            os.path.join(
                ...
                +
                'nla%d_nno%d_bs%d_mnep%d_lr%f_nt%d.pt'
                % (n_layers, n_nodes, batch_size, max_n_epochs, lr, n_random_times_per_data_point))
        ...
        visualization_dir = \
            os.path.join(
                ...
                +
                'nla%d_nno%d_bs%d_nep%d_lr%f_nt%d'
                % (n_layers, n_nodes, batch_size, n_epochs, lr, n_random_times_per_data_point))
